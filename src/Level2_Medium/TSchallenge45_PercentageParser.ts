...
type Rotate<S> = S extends `${infer first}${infer rest}` ?
    `${Rotate<rest>}${first}`
    : S

type CheckLast<T extends string[]> = T extends [first : infer X, rest : infer Y]?
        Rotate<Y> extends `${infer last}${infer rest}`?
            last extends '%'?
                [X,Rotate<rest>,last]
                : [X,`${Rotate<rest>}${last}`,'']
            :[X,'','']
